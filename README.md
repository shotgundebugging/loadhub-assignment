...
Detecție proximitate – pentru fiecare transport cu status READY, sistemul verifică dacă șoferul se află la mai puțin de 100 de metri de locația de start a transportului. Dacă da, transportul este marcat temporar ca „nearby”.
...
De asemenea, odată plecat, nu mai rămâne marcat ca „nearby”.
...
* Pentru calculul distantei dintre 2 puncte se poate folosi RGeo (returneaza distanta in metrii):
  ```ruby
  first_point = RGeo::Geographic.spherical_factory.point(first_address.longitude, first_address.latitude)
  second_point = RGeo::Geographic.spherical_factory.point(second_address.longitude, second_address.latitude)
  
  first_point.distance(second_point) # => 185
  ```

# Frontend (loadhub-fe)
## 3. Pagina integrari GPS
